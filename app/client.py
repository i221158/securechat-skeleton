...
def main():
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            password = getpass.getpass("Enter password: ") # Hides password
            ...
            # 3. Encrypt and send credentials (Req 2.2.4)
            ...
            # 4. Receive and decrypt response (Req 2.2.7)
        ...
        # Try to decrypt an error message if we got to that stage
        if temp_aes_key and "response" not in locals() and "e" in str(e).lower():
             try:
                # This is a bit of a guess, but if the error was in receiving
                # the *final* message, the exception object might be the
                # encrypted bytes. This is not robust, but shows the idea.
